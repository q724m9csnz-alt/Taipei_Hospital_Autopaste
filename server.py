...
def get_checklist():
    raw_data = load_generic_data(CHECKLIST_FILE)
    # --- 修正：確保永遠回傳字典 ---
    # 如果讀取失敗 (回傳 tuple) 或回傳的不是字典 (例如是個 list)，
    # 則回傳一個空的字典，以防止客戶端崩潰。
    if isinstance(raw_data, tuple) or not isinstance(raw_data, dict):
        return jsonify({})
    data = raw_data
    return jsonify(data)
...
def get_doctors():
    raw_data = load_generic_data(DOCTORS_FILE)
    # 如果是錯誤或資料為空，回傳一個包含預設值的有效結構
    if isinstance(raw_data, tuple) or not raw_data:
