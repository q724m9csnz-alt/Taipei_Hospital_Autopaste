...
def load_data():
    """讀取並處理按鈕資料 (data.json)"""
    # 使用統一的、更安全的 load_json_data 函式，如果檔案不存在或為空，則回傳空字典
    raw_data = load_json_data(DATA_FILE, lambda: {})
    # 即使讀取成功，也需要處理重複鍵和排序問題
    return ensure_sort_order(raw_data)
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            if os.path.getsize(file_path) > 0:
                # 對於 data.json，使用 object_pairs_hook 來處理可能的重複鍵
