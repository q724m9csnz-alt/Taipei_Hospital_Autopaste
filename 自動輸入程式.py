...
class AddPatientDialog(simpledialog.Dialog):
    ...
    def body(self, master):
        ...
        detect_btn = tk.Button(master, text=detect_btn_text, command=self.detect_from_screen, font=("Segoe UI", 9), state=detect_btn_state)
        detect_btn.grid(row=4, columnspan=2, pady=10)
        # --- 新增功能：綁定右鍵選單，取代 Shift+點擊 ---
        detect_btn.bind("<Button-3>", self.show_detection_menu)
...
class ChecklistWindow(tk.Toplevel):
    ...
    def show_patient_listbox(self):
        ...
        # 2. 根據 'bed_number' 欄位進行排序
        #    使用 lambda 函式安全地獲取床號，如果沒有床號則視為空字串
        sorted_patients = sorted(patients, key=lambda p: p.get('bed_number', ''))
